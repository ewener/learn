...
* 数组和切片
  ...
  * 切片：值是可变长的。切片的长度可以自动地随着其中元素数量的增长而增长，但不会随着元素数量的减少而减少。切片的底层数据结构中一定会包含数组。
  ...
  * 切片容量的增长：切片无法容纳更多的元素，会生成一个更大容量的切片，然后把原有元素和新元素一起拷贝到新切片中，一般情况下，新切片是原切片容量的2倍，但是当原切片长度大于或等于1024时，会以原容量的1.25被作为新容量的基准。
