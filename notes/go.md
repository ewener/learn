...
> go中唯一一个可以满足并发安全性的类型。可以利用通道在goroutine(go程)之间传递数据。

#####一.声明

> make(chan int,3)    //需要用go内建函数，声明并初始化，chan为通道类型的关键字，需要确定通道类型的元素类型，int为通道的类型元素。第二个参数为通道容量，=0表示无缓冲的通道，>0表示缓冲通道，不能小于0。

注意：缓冲通道和非缓冲通道有不同的数据传递方式。

#####二.通道相当于FIFO队列，通道元素按顺序排列，先进先出。

#####三.元素的发送和接收都要用到接送操作符"<-"，代表传输方向。
...
1. 对于同一个通道，发送操作之间是互斥的，接收操作之间也是互斥的。对于某一个元素，发送完成才能做接收。
2. 发送操作和接收操作中对元素值的处理都是不可分割的。发送操作会发送完整的副本，接收操作接收完成也一定会删除通道的元素。都不会出现半完成状态。
3. 发送操作在完全完成之前会被阻塞。接收操作也是如此。
4. 拷贝的副本是深拷贝。

注意：**元素值从外界进入通道时会被复制。更具体地说，进入通道的并不是在接收操作符右边的那个元素值，而是它的副本。另一方面，元素值从通道进入外界时会被移动。这个移动操作实际上包含了两步，第一步是生成正在通道中的这个元素值的副本，并准备给到接收方，第二步是删除在通道中的这个元素值。**

#####五.发送操作和接收操作在什么时候可能被长时间的阻塞？

正确使用方式造成阻塞：

1. 通道已满，知道通道元素值被接收。
2. 通道已空，接收操作阻塞，直到通道有元素。
3. 非缓冲通道不管发送和接收，一开始就会阻塞。他是同步的传递方式。并且，数据是直接从发送方复制到接收方。中间不会用通道作为中转。
4. 对于缓冲通道，当发送操作在执行的时候发现空的通道中，正好有等待的接收操作，那么也会直接把元素值复制给接收方。

错误使用方式造成阻塞：

1. 通道类型是引用类型，所以它的零值为nil。对于值为nil的通道，不论它的具体类型是什么，对它的发送和接收操作都会永远的处于阻塞。当只声明没有使用make函数初始化时，该变量就是nil。

#####六.发送操作和接收操作在什么时候会引发 panic？

1. 通道关闭：close(c)

2. 关闭一个已经关闭的通道。

   注意：**接收操作是可以感知到通道关闭的，并能够安全退出。这就意味着，当通道关闭了，通道内还有元素，接收方仍从通道重取元素，这个时候是可以取出来的。并且返回的第二个bool类型也一定是true。所以我们千万不要让接收方关闭通道，而应该让发送方做这件事情。**
