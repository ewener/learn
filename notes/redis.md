...
1. 复制过程
   ...
   ![](https://github.com/XwDai/learn/raw/master/notes/image/redis%E4%B8%BB%E4%BB%8E%E8%8A%82%E7%82%B9%E5%BB%BA%E7%AB%8B%E5%A4%8D%E5%88%B6%E6%B5%81%E7%A8%8B.jpg)

2. 数据同步

   1. 全量复制：一般用于初次复制场景。把主节点全部数据一次性发送给从节点，数据量大时，对主从和网络造成很大开销。

      ![](https://github.com/XwDai/learn/raw/master/notes/image/redis%E5%85%A8%E9%87%8F%E5%A4%8D%E5%88%B6%E6%B5%81%E7%A8%8B.jpg)

      > 无盘复制：通过repl-diskless-sync参数控制，默认关闭。RDB不存盘，直接发送给从节点。适用于主节点磁盘性能差但网络带宽富裕场景。注意：无盘复制目前处于试验阶段。

   2. 部分复制：处理主从复制中因网络闪断等原因造成的数据丢失场景，从节点再次连山主节点时，如果条件允许，主节点补发丢失的数据。避免全量复制造成的开销。

      ![](https://github.com/XwDai/learn/raw/master/notes/image/redis%E9%83%A8%E5%88%86%E5%A4%8D%E5%88%B6%E6%B5%81%E7%A8%8B.jpg)

   3. psync

      psync需要的组件支持：

      1. 主从节点各自的复制偏移量(累加的命令字节长度)：如果主从复制偏移量(master_repl_offset-slave_repl_offset)差值较大，可能网络延迟或命令阻塞等引起。

      2. 主节点复制积压缓冲区(repl_backlog_*，固定长度先进先出队列，保存最近已复制数据，默认1M)：写命令发送给从，还会写入缓冲区，用于部分复制和复制命令丢失的数据补救。

      3. 主节点运行id：redis启动后动态分配的一个40位16进制字符串，以它来标识唯一。info server 查看run_id。

         注意：重启后，run_id会变，运行id变化后从节点将做全量复制。

         如何不改变运行id情况下重启？

         debug reload ： 重新加载RDB并保持运行id不变，避免全量复制。但是，这个命令会阻塞，阻塞期间会生成本地快照并清空数据之后再加载RDB文件。对于大数据量和无法容忍阻塞的场景慎用。
      ...
      1. 从节点发送命令
      2. 主节点根据命令参数和自身情况响应结果
         * 回复+FULLRESYNC {runId} {offset}，触发全量复制。
         * 回复+CONTINUE，触发部分复制。
         * 回复+ERR，说明主节点版本低于2.8，无法识别psync，从节点发送sync触发全量复制。

   4. 心跳

      1. 主从节点都有心跳检测机制。
      2. 主节点默认每隔10s对从节点发送ping。repl-ping-slave-period 控制发送频率。
      3. 从节点每隔1s发送replconf ack {offset}，给主节点上报自身当前的复制偏移量。
...
1. 读写分离
   ...
   * 读到过期数据
     ...
     * 惰性删除：主节点处理读取命令会检查超时，超时则执行del并异步发送个从节点。注意，**为了保证复制的一致性，从节点自身永远不会主动删除超时数据**
     * 定时删除：主节点定时采样一定数量键，过期执行del，再同步从节点。3.2版本后从节点在读取数据时会检查过期时间来决定是否返回数据。
